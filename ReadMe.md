...
```bash
python main.py live --symbol GBPUSD --lots 0.01 --sl 8 --tp 12 --mt5-path "C:/Program Files/MetaTrader 5/terminal64.exe"
python main.py live --symbol SYMBOL --lots SIZE --sl STOPLOSS --tp TAKEPROFIT --mt5-path "C:/Program Files/MetaTrader 5/terminal64.exe"
```
...
└────────┬────────┘
    ...
    │                                   │
┌───▼──────────┐   ┌──────────────┐   ┌─────────────┐
│ bot_control  │   │ bot_         │   │ Direct      │
│ .py          │   │ controller   │   │ Command     │
│ (new)        │   │ .py (new)    │   │ (original)  │
└──────────────┘   └──────────────┘   └─────────────┘
